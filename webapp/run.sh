...
docker run --name webapp -p $APP_PORT:$APP_PORT \
  -e "WEBAPP_SERVICE_PORT=$APP_PORT" \
  -e "WEBDB_SERVICE_HOST=$DB_HOST" \
  -e "WEBDB_SERVICE_PORT=$DB_PORT" \
  -e "WEBDB_SERVICE_NAME=$DB_NAME" \
  -e "WEBDB_SERVICE_USERNAME=$DB_USERNAME" \
  -e "WEBDB_SERVICE_PASSWORD=$DB_PASSWORD" \
  -d webapp:$VERSION
